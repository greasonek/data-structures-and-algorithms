...
const sortStarWarsCharacters = (starWarsArr) => {
  // return sortedArr;
  const sortedArr = starWarsArr.sort((p1, p2)=>{
...
const listFoods = (recipe) => {
  let result = [];
  // Solution code here...
  recipe.ingredients.map(foodItem => {
    let deletedVar = foodItem.slice(foodItem.indexOf(' ')).slice(1);
    result.push(deletedVar.slice(deletedVar.indexOf(' ')).slice(1));
  });
  return result;
};
