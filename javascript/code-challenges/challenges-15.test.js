...
const screenForNames = (arr) => {
  // Solution code here...
  const newArr = /^(Mr\.|Mrs\.|Ms\.|Dr\.)\s[a-zA-Z\s]+$/;
  // const str = arr.filter((v) =>{
  //   return newArr.test(v);
  const filteredStr = arr.filter(str => newArr.test(str));
  return filteredStr;
};
...
const sortBy = (property, arr) => {
  ...
  return arr;
  // arr.sort((a,b)=>(a.price > b.price ? 1: -1 && a.name > b.name ? 1: -1));
  // return arr;
};
