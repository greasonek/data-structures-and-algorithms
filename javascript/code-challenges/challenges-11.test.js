...
const count = (target, input) => {
  // Solution code here...
  const numFive = input.reduce((acc, arr)=> {
    let newCount = arr.filter((v)=>v === target).length;
    return acc + newCount;
  }, 0);
  return numFive;
...
const totalSum = (input) => {
  // Solution code here...
  const sum = input.reduce((current, total)=>{
    let totalNum = total.reduce((input, val) =>input + val, 0);
